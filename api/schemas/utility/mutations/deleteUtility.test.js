...
describe('Mutation: deleteUtility', () => {
	let utilityToDelete = {};
	...
	beforeEach(async () => {
		...
		utilityToDelete = (await graphql(schema, query, {}, {})).data.createUtility;
	});

	test('should return success(true) if utility is deleted from database', async () => {
		const query = `mutation {
			deleteUtility(
				id: "${utilityToDelete.id}"
			)
		}`;
		const response = await graphql(schema, query, {}, {});

		expect(response.data.deleteUtility).toBe(true);
	});
